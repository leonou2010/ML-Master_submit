#!/bin/bash
# A/B testing: prompt-only plan/sketch constraints
# Compares plan_constraints.enabled=false (control) vs true (treatment)
#
# Usage: bash experiments/run_plan_constraints_ab.sh <data_dir> <desc_file> [seeds]
# Example: bash experiments/run_plan_constraints_ab.sh /path/to/task /path/to/task.md "0,1,2"
...
RESULTS_DIR="./experiments/results/plan_constraints_ab_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$RESULTS_DIR"

echo "Running A/B tests for plan_constraints"
...
for arm in "${ARMS[@]}"; do
  for seed in "${SEED_ARRAY[@]}"; do
    ...
    python3 main_mcts.py \
      ...
      log_dir="$RESULTS_DIR" \
      agent.steps=50 \
      plan_constraints.enabled="$PLAN_ENABLED" \
