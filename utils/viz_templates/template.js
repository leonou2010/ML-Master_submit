...
function draw() {
  ...
  if (nodes.length == 0) {
    ...
    const defaultNodeIndex = (() => {
      const sel = treeStructData.selection;
      const preferred =
        (sel && sel.post_search && sel.post_search.node != null && sel.post_search.node !== -1)
          ? sel.post_search.node
          : (sel && sel.best_raw && sel.best_raw.node != null && sel.best_raw.node !== -1)
            ? sel.best_raw.node
            : 0;
      const idx = typeof preferred === "number" ? preferred : parseInt(preferred, 10);
      if (Number.isFinite(idx) && idx >= 0 && idx < nodes.length) return idx;
      return 0;
    })();
    selectedNodeIndex = defaultNodeIndex;
    nodes.forEach((n) => (n.selected = false));
    nodes[defaultNodeIndex].selected = true;
    setCodeAndPlan(
      treeStructData.code[defaultNodeIndex],
      treeStructData.plan[defaultNodeIndex],
    );
    // Keep initial selection stable (user can still click other nodes).
    manualSelection = true;
    updateSummaryPanel();
  }
