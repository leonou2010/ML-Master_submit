"""
Post-search selection strategies for ML-Master.
Copied from AIDE's post_search.py to test if robust selection works on MCTS search.
"""
...
from search.mcts_node import MCTSNode as Node  # Alias for compatibility
...
def select_all_methods_at_step(
    ...
    **kwargs
    ...
    Args:
        ...
        top_k: Top-K filter for post-search selection
    ...
    from search.journal import Journal as JournalClass
    from search.mcts_node import MCTSNode

    nodes_up_to_step = [n for n in journal.nodes if hasattr(n, 'step') and n.step <= step]

    if not nodes_up_to_step:
        return {method: None for method in methods}

    # Create a temporary journal view containing only nodes up to this step.
    # NOTE: `search.journal.Journal` stores nodes on the `nodes` attribute (not `_nodes`).
